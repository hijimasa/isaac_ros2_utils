...
class SimLancherWithRestApi(Node):
    def __init__(self):
        super().__init__('sim_launcher_with_rest_api')
        ...
        # Use start_sim_with_rest_api.py instead of start_sim.py
        start_script = os.path.join(
            get_package_share_directory('isaac_ros2_scripts'), 'start_sim_with_rest_api.py')
        command = [
            "bash", python_script, start_script,
            usd_path,
            str(fps),
            str(time_steps_per_second),
            str(real_fps),
            "False",
            str(api_port)
        ]
        print(command)
        self.get_logger().info(f'Starting Isaac Sim with REST API on port {api_port}')
