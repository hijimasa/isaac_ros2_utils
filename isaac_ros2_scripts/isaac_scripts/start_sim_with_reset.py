...
def main():
    ...
    try:
        shm = shared_memory.SharedMemory(name='isaac_sim_reset', size=struct.calcsize('i'), create=False)
    except FileNotFoundError:
        shm = shared_memory.SharedMemory(name='isaac_sim_reset', size=struct.calcsize('i'), create=True)
    ...
    try:
        old_value = struct.unpack_from('i', shm.buf, 0)[0]
        while True:
            read_value = struct.unpack_from('i', shm.buf, 0)[0]
